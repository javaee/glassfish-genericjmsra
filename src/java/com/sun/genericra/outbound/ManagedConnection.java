...
public class ManagedConnection implements javax.resource.spi.ManagedConnection {
    ...
    public ManagedConnection(AbstractManagedConnectionFactory factory,
        ...
        this.connectionEventListenerHandler = new ConnectionEventListenerHandler(
                        this);
        initialize();
    }
    ...
    private javax.jms.Session createXaSession(int sessionType) throws JMSException {
        Session result = null;
        switch (sessionType) {
    ...
    public javax.jms.Session createLocalSession(boolean transacted, int acknowledgeMode, int sessionType) throws JMSException {
        Session result = null;
        switch (sessionType) {
            case Constants.UNIFIED_SESSION :
                Session s = ((Connection)this.physicalJMSCon).createSession(transacted, acknowledgeMode);
                result = s;
                break;
            case Constants.TOPIC_SESSION :
        ...
        if (!(isTransactionInProgress())) {
            ...
            while (iterator.hasNext()) {
                ...
                try {
                    ...
                    debug("Exception while cleaning up connection handle" + jmse);
                }
                iterator.remove();
            }
        } 
    ...
    public void associateConnection(Object connectionHandle)
        ...
        ManagedConnection otherMC = (ManagedConnection) ch
    ...
    public Object getConnection(Subject subject, ConnectionRequestInfo cri)
        ...
        PasswordCredential passedInCred = SecurityUtils.getPasswordCredential(
                        this.mcf, subject, cxRequestInfo);
    ...
    public void sendConnectionErrorEvent(ConnectionHandle handle) {
        if (this.mcf.getClientId() != null) {
            ...
            this.connectionHandles.remove(handle);
            debug("Sending CONN_CLOSED event");
            sendConnectionEvent(ConnectionEvent.CONNECTION_ERROR_OCCURRED, null, handle);
        }
    }
    ...
    void _startLocalTx() throws Exception {
        ...
        if (this.physicalJMSSession == null) {
             ...
             this.physicalJMSSession = createLocalSession(true, Session.AUTO_ACKNOWLEDGE, sessionMode);
        }
        syncSessions();
        debug("The physical session is :"+ this.physicalJMSSession);
    }
